...
!@+   Individual level, NO knowledge of ycells, patches, or cohort struct.
...
!     I.e. GISS GCM computes shc_ycell = shc(mean ycell LAI*vfaction)
!     instead of shc_ycell = mean(shc(patch LAI)*vfraction)
      use yibs_const
...
!@+   Other PFTs for YIBS full documentation.
      integer,intent(in) :: pft
...
!@+   Other YIBS PFTs, see full YIBS documentation.
      integer,intent(in) :: pft
...
!@+      !2.0:  biomass per carbon (kgB/kgC)
      ...
      !use yibs_prescr_veg, only : wooddensity_gcm3
      integer, intent(in) :: pft
      !--- Local ---
...
!@+   Coefficient 0.107 for late-succ hw is approx. mean for all types.
      use yibs_pfts, only : is_conifer,is_hw
      integer, intent(in) :: pft
      real*8, intent(in) :: dbh
      !------
...
!@sum 7/1/2012 - Replaced with different subroutine - NK
      ...
      Cfolmax = Cfol_fn(pft,dbh,h) !g-C/individ
