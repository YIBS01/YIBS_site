...
!@param shw heat capacity of water (at 20 C) (4185 J/kg C)
      ...
      real*8,parameter :: agas = 1d3 * gasc / mair ! = 287.05..
      ...
      !* SOIL / HYDROLOGY *
      !********************
!      integer :: N_DEPTH        !Number of soil layers.  SET IN YIBS_INIT
...
!#define PFT_MODEL_YIBS
#ifdef PFT_MODEL_YIBS
      !************************************************
      !*  YIBS PLANT FUNCTIONAL TYPE CONSTANTS         *
      !************************************************
      ...
      !************************************************************************
      !* CASA SOIL CONSTANTS
      !* THESE SHOULD GO IN THE CASA MODULE BUT NEED TO BE USED IN YIBS_TYPES.F
      !* Array sizes
...
!      integer,parameter :: ptrace = 2  !num. nutrient pools used in CASA resp. routine -PK
     ...
     c             8.5/       ! C:N ratio of passive pool
      ...
      integer, parameter :: npft_oz = 5            ! types of pfts for ozone damage parameters
      real*8,  parameter :: ko3     = 1.67d0       ! ratio of leaf resistance: O3/H2O
      real*8, dimension(npft_oz), parameter :: fo3c = (/   ! (nmol m-2 s-1)
      !    broadleaf,  needleleaf, C3-grass, C4-grass, shrub
     &     1.6d0,  1.6d0,   5.0d0,  5.0d0,   1.6d0/)
      real*8, dimension(npft_oz), parameter :: ah   = (/   ! (mmol-1 m-2)
     &     1.5d-1, 7.5d-2,  1.4d0,  7.35d-1, 1.0d-1/)
      real*8, dimension(npft_oz), parameter :: al   = (/   ! (mmol-1 m-2)
     &     4.0d-2, 2.0d-2,  2.5d-1, 1.3d-1,  3.0d-2/)

#ifndef PFT_MODEL_YIBS         
      integer, dimension(N_PFT), parameter  :: io3d = (/   ! look-up table from YIBS to O3dep PFT
     &     5, 3, 5, 3, 1, 2, 1, 3
#ifdef PFT9_C4
     &    ,4
#endif
     ...
     &     1, 1, 2, 2, 1, 1, 1, 2, 5, 5, 3, 4, 3, 3, 4, 3/)
      ...
      !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
            character*13, parameter :: YIBS_cpool_title(N_BPOOLS) =
     &     (/
     &     'FOLIAGE      '
     &     ,'SAPWOOD      '
     &     ,'HARDWOOD     '
