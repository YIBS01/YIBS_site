      module yibs_pfts
!@sum Parameter set for YIBS default supported 16 plant functional types
!@+   tailored to Fluxnet site YIBS_1D runs.
      ...
      !***************************************************
      !*      YIBS PLANT FUNCTIONAL TYPES                 *
      !***************************************************

      character*50, parameter :: YIBS_title(N_COVERTYPES) =
     &     (/
...
!##### TEMPORARY HACK - YK #####
!to avoid the conflict in yibs_prescribed_drv.f90, using CROPS!
      integer, parameter :: CROPS = 15
!##### END OF TEMPORARY HACK #####
      
      ! other parameters needed for YIBS to compile
      ...
      !***************************************************
      !Temp values for YIBS pfts (See yibs_const.f for types)
...
!YK     &     1.8d0,41.0d0, 40.d0, 0.2d0, 0.075d0, 1,
     &     pftype(1,.true.,1,-153.d0, .60d0, .29d0, 1.1d0, 9.9d0, 0.5d0, 
     &     1.8d0,41.0d0, 40.d0, 0.2d0, 0.075d0, 1,
     &     0.0347d0, 1.560d0, 0.0816d0, 2.306d0, 34.62d0, -0.0232d0,
     &     2.71d-2,1.87d-4),
     ! !* 3 - evergreen needleleaf early successional
...
!     &     pftype(1,.true.,1,-500.d0, .50d0, .29d0, 1.5d0, 12.4d0,0.6d0, 
     ...
     &     6.07d-2,2.23d-4),
     ! !* 6 - cold deciduous broadleaf late successional
     &     pftype(1,.true.,1,-500.d0, .50d0, .29d0, 1.5d0,8.30d0,0.6d0, !old SLA 11.5, 0.6
     &     1.20d0, 58.0d0, 50.d0, 0.2d0, 0.093d0, 2,
...
!YKIM - sstar = 0.34 & swilt 0.28
     ...
     &     1.60d-2,1.58d-4),
     ! !* 12 - C4 grass
     &     pftype(2,.false.,3,-2030.d0,0.5d0,0.3d0,1.3d0,22.0d0,0.6d0,
     &     1.5d0, UNDEF, 50.d0, 0.1d0, 0.0d0, 4,
     &     0.0800d0, 1.000d0, 0.00001d0, 1.000d0, 0.4778d0, -0.75d0,
     &     0.0d0,0.0d0),
     ! !* 13 - C3 grass - annual 
...
!YKIM - sstar = 0.65 & swilt 0.27
     &     pftype(1,.false.,3,-2030.d0,.65d0,.27d0,1.5d0, 21.6d0, 1.2d0,
     &     1.5d0, UNDEF, 50.d0, 0.1d0, 0.0d0, 4,
     &     0.0800d0, 1.000d0, 0.00001d0, 1.000d0, 0.4778d0, -0.75d0,
     &     1.60d-2,1.58d-4),
     ! !* 14 - arctic C3 grass
...
!     &     ( /N_PFT,NPOOLS-NLIVE,N_CASA_LAYERS/ )
     ...
     &     ,undef,undef,undef,undef,undef,0.54d0 /)
      real*8, DIMENSION(N_PFT), parameter :: a0h = !Later move to yibs_pfts16.f
