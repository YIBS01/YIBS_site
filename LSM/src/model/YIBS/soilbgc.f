...
!@auth P.Kharecha, N.Y.Kiang
      ...
      real*8, intent(in) :: dtsec !main yibs time step (s)
...
!      Soiltemp(:) = pp%cellptr%Soiltemp(:)  !soil temp, texture vary by cell
      call Soillayer_convert_YIBS(pp%cellptr%Soilmoist(:), SOILDEPTH_m, 
     &     Soilmoist) 
!      Soilmoist(:) = Soilmoist(:)*pp%cellptr%soil_Phi !Convert from rel.sat. to vol fraction. TEMPORARY TESTS
      call Soillayer_convert_YIBS(pp%cellptr%Soiltemp(:), SOILDEPTH_m, 
     &     Soiltemp) 
...
!***********************************************************************
      subroutine Soillayer_convert_YIBS(s, depthm, Savglayer) 
...
!@+   average values for 0-30 cm and, optional, 30-100 cm.
      ...
      !---Local-----
#ifdef NCASA2
      real*8, parameter :: YIBSD(N_CASA_LAYERS) = ( / 0.30d0, 1.0d0 /)
#else
      real*8, parameter :: YIBSD(N_CASA_LAYERS) = ( / 0.30d0 /)
#endif
      integer :: k,ek
      real*8 :: sk,ddk,depthup
      real*8 :: savg, dsum
      logical :: yibsdpassed !Flag for if YIBS soil layer depth was passed.
      ...
      do ek = 1,N_CASA_LAYERS
         ...
         dsum = 0.d0
         ddk = depthm(k)
         do while ((.not.yibsdpassed).and.(k.le.N_DEPTH))
            if (k.eq.1) then !Top layer
               depthup = 0.d0
            else !Lower layers
               depthup = depthm(k-1)
            endif
            if (depthm(k).le.YIBSD(ek)) then
               ddk = depthm(k)- depthup
            else    
                ddk = YIBSD(ek) - depthup
                yibsdpassed = .true.
            endif
            savg = savg + ddk*s(k)
            dsum = dsum + ddk
      ...
      end subroutine Soillayer_convert_YIBS
